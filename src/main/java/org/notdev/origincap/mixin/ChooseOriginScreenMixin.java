...
public abstract class ChooseOriginScreenMixin extends OriginDisplayScreen {
    ...
    // enums not allowed annoying since this rlly shouldn't be its own class since it's a mixin and I am kinda using mixins in a undesirable way
    // 0 = inactive 1 = awaiting 2 = active
    private int buttonState;


    private ChooseOriginScreenMixin(Text title, boolean showDirtBackground) {
    ...
    protected Element init(ChooseOriginScreen instance, Element element) {
        ...
        // showOrigin which I override below which also changes the state of the button is run before
        // this.selectButton is assigned so I need to make sure to have it update the look of the button
        // after assigning
        updateButtonState();
        ...
        CapHandler.onGetResult = (packet) -> {
            // origin and layer recieved from packet is not equal to the current origin on screen
            // maybe switched screens too fast and packet didnt come back in time
            // just ignore the packet because new origin selected has also sent its own request to the server
            if(!equalsCurrOrigin(packet.layerId(),packet.originId())) return null;
            if(packet.isFull()) setButtonState(0); // 0 = inactive
            else setButtonState(2); // 2 = active
    ...
    private boolean equalsCurrOrigin(String layerId, String originId) {
        return layerList.get(currentLayerIndex).getIdentifier().toString().equals(layerId) &&  getCurrentOriginInternal().getIdentifier().toString().equals(originId);
    }
    ...
    public void showOrigin(Origin origin, OriginLayer layer, boolean isRandom) {
        ...
        ClientPlayNetworking.send(new C2S_IsOriginFullCheck(layer.getIdentifier().toString(), origin.getIdentifier().toString()));
        System.out.println("Sent packet to server");
    }


    /**
     * sets the button state and interact-ability and changes the button appearance to reflect its state
     *
     * @param state 0 = inactive <br>1 = awaiting response from server <br>2 = full
     **/
    private void setButtonState(int state) {
        System.out.println(state);
        this.buttonState = state;
        if(this.selectButton != null) updateButtonState();
    }
